...
class Auth extends CI_Controller
{
    ...
    public function update_password()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            redirect(base_url('auth/reset_password_form/' . $token));
        }
    ...
    public function register_process()
    {
        ...
        $this->form_validation->set_rules('cb_phone', 'Phone Numbexr', 'required');
        ...
        $this->form_validation->set_rules('cb_pendidikan_s1', 'Pendidikan S1', 'required');
        $this->form_validation->set_rules('university_year', 'Tahun Masuk', 'required');
        $this->form_validation->set_rules('university_year_grad', 'Tahun Lulus', 'required');
        $this->form_validation->set_rules('cb_pendidikan_sp', 'Pendidikan Spesialis', 'required');
        $this->form_validation->set_rules('spc_year', 'Tahun Masuk Spesialis', 'required');
        $this->form_validation->set_rules('spc_year_grad', 'Tahun Lulus Spesialis', 'required');
        ...
        if ($this->form_validation->run() == FALSE) {
            // Form validation failed, reload the registration form with errors
            $this->session->set_flashdata("error", "mohon isi semua field");
            ...
            $data = array(
                ...
                'cb_phone' => $cb_phone,
                'cb_pendidikan_s1' => $cb_pendidikan_s1,
                'university_year' => $university_year,
                'university_year_grad' => $university_year_grad,
                'cb_pendidikan_sp' => $cb_pendidikan_sp,
                'spc_year' => $spc_year,
                'spc_year_grad' => $spc_year_grad,
                'password' => md5($password),
                'middlename' => $password
            );
            //File
            ...
            if ($emailexist < 1) {
                ...
                $this->load->library('email', $configmail);
                $this->email->from('admin@ikortijaya.org', 'Admin IKORTIJAYA');
                $this->email->to($this->input->post('email'));
                $this->email->subject('Pembayaran Iuran IKORTIJAYA - No Reply');
                $this->email->message('Yth. Rekan Sejawat,<br>
                <br>
                <br>
                Selamat datang di website Ikatan Ortodontis Indonesia Pengurus Wilayah Jakarta Raya (IKORTI Pengwil Jaya).
                <br>
                Akun anda di website IKORTI Pengwil Jaya akan diverifikasi terlebih dahulu. Harap menunggu email verifikasi oleh kami. 
                <br>
                <br>
                Terima kasih,<br>
                ...
                // For Admin Notif
                $this->load->library('email', $configmail);
                $this->email->from('system@ikortijaya.org', 'System IKORTIJAYA');
                $this->email->to('admin@ikortijaya.org');
                $this->email->subject('Pendaftaran Baru');
                $this->email->message('Yth. Pengurus IKORTI Pengwil Jaya,
                <br>
                <br>
                Anggota baru telah mendaftar ke website anda.
                <br>
                <br>
                Nama                : '.$fullname.'<br>
                Nama dengan gelar	: '.$fullname_title.'<br>
                Jenis kelamin		: '.$gender.'<br>
                Tempat/ Tanggal lahir	: '.$place_birth.' - '.$date_birth.'<br>
                Alamat email		: '.$email.'<br>
                No. telp.			: '.$cb_phone.'<br>
                NPA			: '.$npa.'<br>
                Asal universitas (Sp.)	: '.$cb_pendidikan_sp.'<br>
                Tahun masuk 		: '.$spc_year.'<br>
                Tahun lulus 		: '.$spc_year_grad.'<br>
                <br>
                Harap melakukan verifikasi keanggotaan. Terima kasih.');
                ...
                $this->session->set_flashdata("message", "Selamat, anda telah terdaftar. Silakan tunggu untuk proses verifikasi");
    ...
    public function profile()
    {
        ...
        if ($this->session->userdata('member')) {
            ...
            $data['datamember'] = $this->db->get_where('member', array('id' => $var['id']))->result()[0];
            $data['datacertificate'] = $this->db->get_where('sertifikat', array('email' => $var['email']))->result();
            // print_r($data['datamember']);
            // die();
            $this->load->view('templatemember', $data);
        } else {
